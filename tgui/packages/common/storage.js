...
const INDEXED_DB_NAME = 'zeroonyx-tgui'
...
class HubStorageBackend {
  ...
  async get (key) {
    const value = await window.hubStorage.getItem('zeroonyx-' + key)
  ...
  set (key, value) {
    window.hubStorage.setItem('zeroonyx-' + key, JSON.stringify(value));
  }

  remove (key) {
    window.hubStorage.removeItem('zeroonyx-' + key)
  }
